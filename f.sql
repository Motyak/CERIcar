...
-- Permet de remplir la table temporaire 
...
begin
	...
	loop
		...
		if res.arrivee=v_const_arrivee then
            -- recuperation de l'id proposition
            select id into v_id_prop from prop_tmp limit 1;
            ...
            for i in 1 .. array_length(v_branche,1)
            loop
                -- si dernier voyage de la branche..
                if(i = array_length(v_branche,1)) then
            ...
            update prop_tmp set id=v_id_prop;
            -- clear la branche
            delete from branche_tmp;
